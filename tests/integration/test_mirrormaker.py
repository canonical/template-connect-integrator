...
async def test_remove_relation(ops_test: OpsTest):
    ...
    await produce_messages(ops_test, KAFKA_APP, topic="arnor", no_messages=200)
    logging.info("Extra 200 messages produced to topic arnor, total 300")

    await asyncio.sleep(10)

    # Check that the messages can didn't increment on the passive cluster
    await assert_messages_produced(ops_test, KAFKA_APP_B, topic="arnor", no_messages=100)

    # Reset state for next tests
    await cleanup_mm_topics(ops_test, KAFKA_APP_B, extra_topics=["arnor"])
